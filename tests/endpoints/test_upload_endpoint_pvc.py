"""Tests for data upload endpoint with PVC storage."""
...
def generate_multi_input_payload(
    n_rows: int,
    n_input_cols: int,
    n_output_cols: int,
    datatype: str,
...
def count_rows_with_tag(model_name: str, tag: str) -> int:
    """Count rows with a specific tag in metadata."""
    metadata_df = asyncio.run(ModelData(model_name).get_metadata_as_df())
    return int(metadata_df["tags"].apply(lambda tags: tag in tags).sum())


def get_metadata_ids(model_name: str) -> list[str]:
    """Count rows with a specific tag in metadata."""
    metadata_df = asyncio.run(ModelData(model_name).get_metadata_as_df())
...
class TestUploadEndpointPVC(unittest.TestCase):
    """Test data upload endpoint with PVC storage backend."""

    def setUp(self) -> None:
        """Set up PVC storage environment with temporary directory."""
        # Save original environment variable
        self.original_storage_folder = os.environ.get("STORAGE_DATA_FOLDER")
        ...
        def restore_environment() -> None:
            if self.original_storage_folder is None:
                os.environ.pop("STORAGE_DATA_FOLDER", None)
            else:
                os.environ["STORAGE_DATA_FOLDER"] = self.original_storage_folder
    ...
    def test_upload_data(self) -> None:
        """Test data upload with various row/column/datatype combinations."""
        ...
        for idx, (n_input_rows, n_input_cols, n_output_cols, datatype) in enumerate(
            ...
            with self.subTest(
                ...
                assert str(n_input_rows) in response.text
                assert inputs is not None, "Input data not found in storage"
                assert outputs is not None, "Output data not found in storage"

                assert inputs is not None  # Type narrowing for mypy
                assert outputs is not None  # Type narrowing for mypy
                assert len(inputs) == n_input_rows, "Incorrect number of input rows"
                assert len(outputs) == n_input_rows, "Incorrect number of output rows"
    ...
    def test_upload_multi_input_data(self) -> None:
        ...
        for n_rows, n_input_cols, n_output_cols, datatype in itertools.product(
            ...
            with self.subTest(
                ...
                input_column_names, output_column_names, _ = asyncio.run(
                ...
                assert inputs is not None  # Type narrowing for mypy
                assert outputs is not None  # Type narrowing for mypy
                assert len(inputs) == n_rows, "Incorrect number of input rows"
                assert len(outputs) == n_rows, "Incorrect number of output rows"
                assert len(input_column_names) == n_input_cols, (
                    "Incorrect number of input columns"
                )
                assert len(output_column_names) == n_output_cols, (
                    "Incorrect number of output columns"
                )
                assert len(input_column_names) >= MIN_INPUT_COLUMNS, (
    ...
    def test_upload_mismatched_row_counts(self) -> None:
        ...
        assert "number of" in response_text.lower()
    ...
    def test_upload_multiple_tagging(self) -> None:
        ...
        assert input_rows == n_payload1 + n_payload2, "Incorrect total number of rows"
    ...
    def test_upload_tag_that_uses_protected_name(self) -> None:
        ...
        expected_msg = (
            f"The tag prefix '{TRUSTYAI_TAG_PREFIX}' is reserved for internal "
            f"TrustyAI use only. Provided tag '{invalid_tag}' violates this "
            f"restriction."
        )
