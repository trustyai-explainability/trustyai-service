...
def _make_storage_mocks(
    ...
    def read_data(
        name: str, *, start_row: int | None = None, n_rows: int | None = None
    ) -> np.ndarray:
        _ = start_row
        end = None if n_rows is None else (start_row or 0) + n_rows
        begin = start_row or 0
        if "metadata" in name:
            return metadata[begin:end]
        if "outputs" in name:
            return output_rows[begin:end]
        return input_rows[begin:end]
...
def make_compute_endpoint_test(
    ...
    def test_impl(_: object) -> None:
        ...
        with (
            patch(
                f"{module}.run_local_worker", new_callable=AsyncMock
            ) as mock_to_thread,
