...
- Storage is loaded through ``service.data.local_explanation``'s global-storage
  accessor
...
- ``service.data.local_explanation.get_global_storage_interface`` (patch the
  loader boundary directly)
- An availability flag (e.g. ``_LIME_AVAILABLE``) checked at handler entry

Patch targets for every factory function (the algorithm module is selected from
the endpoint path):
- ``{DATA_MODULE}.get_global_storage_interface``  — direct local-data storage
- ``{LIME_MODULE|SHAP_MODULE}.{_LIME_AVAILABLE|_SHAP_AVAILABLE}``  — availability guard
- No ModelData patch is required; local data loading bypasses that lifecycle.
"""
...
LIME_MODULE = "trustyai_service.endpoints.explainers.local_lime"
SHAP_MODULE = "trustyai_service.endpoints.explainers.local_shap"
DATA_MODULE = "trustyai_service.service.data.local_explanation"
...
def _make_storage_mocks(
    ...
    # Metadata: one target row followed by organic rows.
    metadata = np.array(
        [[pred_id, "2025-01-01T00:00:00", 1.0, []]]
        + [
            [f"organic-{index}", "2025-01-01T00:00:00", 1.0, []]
            for index in range(n_organic_rows)
        ],
        dtype="O",
    )
    ...
    input_rows = np.vstack(
        [input_row, rng.standard_normal((n_organic_rows, n_features))]
    )
    output_rows = (
        np.vstack(
            [
                np.zeros((1, len(output_names))),
                np.column_stack([organic_data[name] for name in output_names]),
            ]
        )
        if output_names
        else np.empty((n_organic_rows + 1, 0))
    )

    def read_data(
        name: str, start_row: int = 0, n_rows: int | None = None
    ) -> np.ndarray:
        """Return the requested bounded storage slice."""
        end = None if n_rows is None else start_row + n_rows
        if "metadata" in name:
            return metadata[start_row:end]
        if "outputs" in name:
            return output_rows[start_row:end]
        return input_rows[start_row:end]

    mock_si.read_data = AsyncMock(side_effect=read_data)
    ...
    mock_si.dataset_rows = AsyncMock(return_value=n_organic_rows + 1)
...
def make_explainer_unavailable_test(
    ...
    Args:
        ...
        availability_flag: Full dotted path to the availability bool, e.g.
            ``"trustyai_service.endpoints.explainers.local_lime._LIME_AVAILABLE"``.
    ...
    def test_impl() -> None:
        ...
        assert detail["code"] == "dependency_unavailable", (
            f"{explainer_name}: expected dependency_unavailable code"
        )
...
def make_model_not_found_test(
    ...
    Args:
        ...
        request_payload: Valid JSON body for the POST request.
        availability_flag: Full dotted path to the availability bool (e.g.
            ``"...local_shap._SHAP_AVAILABLE"``).  When provided, the flag
    ...
    def _check_not_found_404(mock_si: MagicMock) -> None:
        ...
        detail = response.json()["detail"]
        assert detail["code"] == "data_missing", (
            f"{explainer_name}: expected data_missing code"
        )

    def _make_test(avail_flag: str | None) -> Callable[[object], None]:
        @patch(f"{DATA_MODULE}.get_global_storage_interface")
        def test_impl_no_avail(mock_global: MagicMock) -> None:
            mock_si = MagicMock()
            mock_si.dataset_exists = AsyncMock(return_value=False)
        ...
        @patch(f"{DATA_MODULE}.get_global_storage_interface")
        def test_impl_with_avail(mock_global: MagicMock) -> None:
            mock_si = MagicMock()
            mock_si.dataset_exists = AsyncMock(return_value=False)
            mock_global.return_value = mock_si
            _check_not_found_404(mock_si)
...
def make_prediction_id_not_found_test(
    ...
    def _make_test(avail_flag: str | None) -> Callable[[object], None]:
        ...
        def _check_pred_id_not_found_400(mock_si: MagicMock) -> None:
            ...
            detail = response.json()["detail"]
            assert detail["code"] == "data_invalid", (
                f"{explainer_name}: expected data_invalid code"
            )

        @patch(f"{DATA_MODULE}.get_global_storage_interface")
        def test_impl_no_avail(mock_global: MagicMock) -> None:
            mock_si = MagicMock()
            mock_si.dataset_rows = AsyncMock(return_value=1)
            mock_global.return_value = mock_si
...
def make_compute_endpoint_test(
    ...
    """200 contract test: valid request returns all expected response keys.

    The explainer computation (LIME/SHAP) is bypassed by patching
    ``run_local_worker`` to return ``compute_thread_return`` directly.  This
    helper is for endpoint response-contract tests only; real-model tests must
    exercise the provider through a fake KServe HTTP server and must not use
    this bypass.
    ...
    Args:
        ...
        compute_thread_return: Return value from mocked ``run_local_worker``;
            it must match the current handler's worker-result contract.
        availability_flag: Full dotted path to the availability bool, e.g.
            ``"trustyai_service.endpoints.explainers.local_lime._LIME_AVAILABLE"``.
    ...
    module = SHAP_MODULE if endpoint_path.endswith("/shap") else LIME_MODULE

    @patch(availability_flag, new=True)
    def test_impl() -> None:
        mock_si, _mock_ds = _make_storage_mocks(pred_id, feature_names, output_names)
        with (
            patch(f"{module}.run_local_worker", new_callable=AsyncMock) as mock_worker,
            patch(f"{DATA_MODULE}.get_global_storage_interface") as mock_global,
        ):
            mock_global.return_value = mock_si
            mock_worker.return_value = compute_thread_return
            response = client.post(endpoint_path, json=request_payload)
