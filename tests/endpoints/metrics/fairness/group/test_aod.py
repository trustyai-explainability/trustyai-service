...
from src.endpoints.metrics.fairness.group.aod import AODMetricRequest, router

app = FastAPI()
app.include_router(router)
client = TestClient(app)

MODULE = "src.endpoints.metrics.fairness.group.aod"
...
class TestAODComputeEndpoint:
    ...
    def test_compute_returns_valid_response(self, mock_ds: MagicMock) -> None:
        ...
        response = client.post("/metrics/group/fairness/aod", json=_base_request_payload())

        assert response.status_code == 200
        data = response.json()
        assert data["name"] == "AOD"
    ...
    def test_compute_with_custom_delta(self, mock_ds: MagicMock) -> None:
        ...
        response = client.post("/metrics/group/fairness/aod?delta=0.05", json=_base_request_payload())
    ...
    @patch(f"{MODULE}.get_data_source")
    def test_compute_identical_predictions_gives_zero_aod(self, mock_ds: MagicMock) -> None:
        """When predictions match ground truth for both groups, AOD should be ~0."""
        df = pd.DataFrame({
...
class TestAODDefinitionEndpoint:
    def test_definition_returns_name_and_description(self) -> None:
        response = client.get("/metrics/group/fairness/aod/definition")
        assert response.status_code == 200
        data = response.json()
        assert "Average Odds Difference" in data["name"]
    ...
    def test_interpret_value(self) -> None:
        ...
        response = client.post("/metrics/group/fairness/aod/definition", json=payload)
        assert response.status_code == 200
        assert "interpretation" in response.json()


class TestAODScheduleEndpoints:
    ...
    def test_schedule_returns_request_id(self, mock_sched_fn: MagicMock) -> None:
        ...
        response = client.post("/metrics/group/fairness/aod/request", json=_base_request_payload())
    ...
    @patch(f"{MODULE}.get_prometheus_scheduler")
    def test_schedule_register_exception(self, mock_sched_fn: MagicMock) -> None:
        mock_sched = MagicMock()
        mock_sched.register = AsyncMock(side_effect=Exception("Connection failed"))
        mock_sched_fn.return_value = mock_sched

        response = client.post("/metrics/group/fairness/aod/request", json=_base_request_payload())

        assert response.status_code == 500
        assert "error scheduling metric" in response.json()["detail"].lower()
    ...
    def test_delete_schedule(self, mock_sched_fn: MagicMock) -> None:
        ...
        response = client.request("DELETE", "/metrics/group/fairness/aod/request", json={"requestId": test_uuid})
    ...
    def test_delete_invalid_uuid(self, mock_sched_fn: MagicMock) -> None:
        ...
        response = client.request("DELETE", "/metrics/group/fairness/aod/request", json={"requestId": "not-a-uuid"})
    ...
    def test_delete_scheduler_unavailable(self, mock_sched_fn: MagicMock) -> None:
        ...
        response = client.request(
            "DELETE",
            "/metrics/group/fairness/aod/request",
            json={"requestId": "123e4567-e89b-12d3-a456-426614174000"},
        )
        assert response.status_code == 500


class TestAODListEndpoints:
    @patch(f"{MODULE}.get_prometheus_scheduler")
    def test_list_empty(self, mock_sched_fn: MagicMock) -> None:
        ...
        response = client.get("/metrics/group/fairness/aod/requests")
    ...
    def test_list_exception(self, mock_sched_fn: MagicMock) -> None:
        ...
        response = client.get("/metrics/group/fairness/aod/requests")
        assert response.status_code == 500
        assert "error listing requests" in response.json()["detail"].lower()


class TestAODDeprecatedEndpoints:
    @patch(f"{MODULE}.get_data_source")
    def test_deprecated_compute(self, mock_ds: MagicMock) -> None:
        ...
        response = client.post("/aod", json=_base_request_payload())
        assert response.status_code == 200
        assert response.json()["name"] == "AOD"

    def test_deprecated_definition(self) -> None:
        response = client.get("/aod/definition")
        assert response.status_code == 200
        assert "Average Odds Difference" in response.json()["name"]
    ...
    def test_deprecated_schedule(self, mock_sched_fn: MagicMock) -> None:
        ...
        response = client.post("/aod/request", json=_base_request_payload())
        assert response.status_code == 200
    ...
    def test_deprecated_delete(self, mock_sched_fn: MagicMock) -> None:
        ...
        response = client.request("DELETE", "/aod/request", json={"requestId": "123e4567-e89b-12d3-a456-426614174000"})
        assert response.status_code == 200
    ...
    def test_deprecated_list(self, mock_sched_fn: MagicMock) -> None:
        ...
        response = client.get("/aod/requests")
        assert response.status_code == 200
        assert "requests" in response.json()


class TestAODMetricRequest:
    def test_model_validator_sets_metric_name(self) -> None:
        ...
        assert request.metric_name == "AOD"
