...
def test_schedule_error_factory_supports_register_side_effect() -> None:
    ...
    assert "register_side_effect" in params, (
        "Factory should support register_side_effect parameter"
    )
    assert params["register_side_effect"].default is None, (
        "register_side_effect should default to None"
    )


def test_delete_error_factory_supports_delete_side_effect() -> None:
    ...
    assert "delete_side_effect" in params, (
        "Factory should support delete_side_effect parameter"
    )
    assert params["delete_side_effect"].default is None, (
        "delete_side_effect should default to None"
    )


def test_register_side_effect_behavior() -> None:
...
def test_delete_side_effect_behavior() -> None:
...
def test_various_exception_types() -> None:
...
def test_register_side_effect_none_means_no_exception() -> None:
...
def test_delete_side_effect_none_means_no_exception() -> None:
...
def test_factory_docstring_documents_new_parameters() -> None:
    ...
    assert schedule_doc is not None, "Schedule factory should have docstring"
    assert "register_side_effect" in schedule_doc, (
        "Schedule factory should document register_side_effect"
    )
    assert "Exception to raise" in schedule_doc, (
        "Should explain what register_side_effect does"
    )

    delete_doc = factory.make_delete_endpoint_error_test.__doc__
    assert delete_doc is not None, "Delete factory should have docstring"
    assert "delete_side_effect" in delete_doc, (
        "Delete factory should document delete_side_effect"
    )
    assert "Exception to raise" in delete_doc, (
        "Should explain what delete_side_effect does"
    )
