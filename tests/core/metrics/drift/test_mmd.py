...
class TestMMDValidation:
    ...
    def test_import_error_when_goodpoints_missing(self) -> None:
        """Clear ImportError when goodpoints is not installed."""
        from unittest.mock import patch  # noqa: PLC0415

        from src.core.metrics.drift.mmd import _import_goodpoints  # noqa: PLC0415

        rng = np.random.default_rng(0)
        ref = rng.standard_normal((50, 2))
        cur = rng.standard_normal((50, 2))

        _import_goodpoints.cache_clear()
        try:
            with (
                patch.dict("sys.modules", {"goodpoints": None, "goodpoints.ctt": None}),
                pytest.raises(ImportError, match="goodpoints"),
            ):
                MMD.compute(ref, cur)
        finally:
            _import_goodpoints.cache_clear()
