...
class TestComputeLimeTupleShape:
    """Bug 1: Verify compute_lime_explanation returns correct tuple shape.

    The handler at local_explainer.py line ~440 unpacks as:
        weights, r2, local_pred, intercept = await asyncio.wait_for(...)

    The CI contract in the endpoint handler expects _compute() to return:
        (weights, r2, local_pred, intercept, lower, upper)

    This test validates the actual return type from compute_lime_explanation.
    """

    def test_compute_returns_four_element_tuple(self) -> None:
        """compute_lime_explanation returns (weights, r2, local_pred, intercept)."""
        rng = np.random.default_rng(42)
        x_train = rng.standard_normal((100, 5))
        y_train = 3 * x_train[:, 0] + rng.standard_normal(100) * 0.1
        ...
        # Bug 1: Verify tuple has exactly 4 elements
        assert isinstance(result, tuple), f"Expected tuple, got {type(result)}"
        assert len(result) == 4, (
            f"Expected 4-element tuple (weights, r2, local_pred, intercept), "
            f"got {len(result)}-element tuple"
        )

        weights, r2, local_pred, intercept = result
        assert isinstance(weights, list)
        assert all(isinstance(w, tuple) and len(w) == 2 for w in weights)
        assert isinstance(r2, float)
        assert isinstance(local_pred, float)
