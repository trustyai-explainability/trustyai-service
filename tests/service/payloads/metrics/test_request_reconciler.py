"""Tests for metric request reconciler."""
...
class MockReconcilableRequest(BaseMetricRequest):
    ...
    def __init__(self, model_id: str = "test_model") -> None:
        """Initialize mock request with reconcilable feature and output fields."""
        # Set class-level reconciler matcher descriptors BEFORE Pydantic init.
        # This ensures Pydantic stores the instance attributes in __dict__
        # (not __pydantic_extra__), so they aren't shadowed by the class Mocks.
        type(self).protected_attribute = Mock()
        type(self).protected_attribute._reconciler_matcher = ReconcilerMatcher(
            "get_protected_attribute",
        )
        type(self).favorable_outcome = Mock()
        type(self).favorable_outcome._reconciler_matcher = ReconcilerMatcher(
            "get_outcome_name",
        )
    ...
    def get_protected_attribute(self) -> str:
        """Return the protected attribute field name."""
        return "gender"

    def get_outcome_name(self) -> str:
        """Return the outcome field name."""
        return "income"

    def retrieve_tags(self) -> dict[str, str]:
        """Return test tags for the request."""
        return {"test": "value"}
...
class TestRequestReconciler:
    ...
    def test_reconcile_with_metadata_reconciles_feature(
        ...
        RequestReconciler.reconcile_with_metadata(
            ...
            storage_metadata=mock_storage_metadata,
        )
    ...
    def test_reconcile_with_multiple_value_nodes(
        ...
        assert len(reconciled) == EXPECTED_RECONCILED_COUNT
    ...
    def test_reconcile_with_non_callable_name_provider(
        ...
        with pytest.raises(
            IllegalArgumentError,
            match="name-providing-method that is not callable",
        ):
            RequestReconciler.reconcile_with_metadata(request, mock_storage_metadata)
    ...
    def test_reconcile_ignores_none_field_values(
        ...
        # Set field to None
        request.protected_attribute = None  # type: ignore[assignment]
    ...
    def test_reconcile_with_missing_schema_field_raises_error(
        ...
        with pytest.raises((ValueError, KeyError, Exception)):  # Field not found error
    ...
    async def test_reconcile_logs_success(
        ...
        with patch(
            ...
            mock_logger.info.assert_called_with(
                "Reconciled request for model %s",
                mock_request.model_id,
            )
