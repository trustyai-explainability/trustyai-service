...
@pytest.mark.asyncio
async def test_loader_uses_injected_storage_without_model_lock_dependency() -> None:
    """Keep local data loading independent from storage-concurrency helpers."""
    ...
    assert "get_model_lock" not in load_local_explanation_data.__globals__
...
@pytest.mark.asyncio
async def test_loader_reports_missing_organic_background_as_not_found() -> None:
    """Classify a model with no usable organic rows as missing local data."""
    storage = _VirtualStorage(
        target_indices={0},
        organic_indices=set(),
    )

    with pytest.raises(LocalDataNotFoundError, match="organic background"):
        await load_local_explanation_data(
            "model",
            "target",
            1,
...
async def test_loader_detects_duplicate_id_after_the_former_boundary() -> None:
    ...
    with pytest.raises(LocalDataError, match="ambiguous") as exc_info:
        await load_local_explanation_data(
            "model",
            "target",
            1,
    ...
    assert map_error(exc_info.value).code == "data_invalid"
...
async def test_loader_rejects_missing_prediction_id() -> None:
    ...
    with pytest.raises(LocalDataNotFoundError, match="not found") as exc_info:
        await load_local_explanation_data(
            "model",
            "target",
            1,
    ...
    assert map_error(exc_info.value).code == "data_missing"
...
async def test_loader_rejects_duplicate_input_aliases() -> None:
    ...
    with pytest.raises(LocalDataError, match="aliases"):
        await load_local_explanation_data(
            "model",
            "target",
            1,
...
async def test_surrogate_mode_reports_missing_stored_outputs_with_typed_error() -> None:
    ...
    assert isinstance(exc_info.value, LocalDataNotFoundError)
    assert map_error(exc_info.value).code == "data_missing"
...
async def test_loader_rejects_input_metadata_row_misalignment() -> None:
    ...
    with pytest.raises(LocalDataError, match="input and metadata"):
        await load_local_explanation_data(
            "model",
            "target",
            1,
...
async def test_loader_rejects_invalid_model_feature_arrays(inputs: np.ndarray) -> None:
    ...
    with pytest.raises(LocalDataError, match="input"):
        await load_local_explanation_data(
            "model",
            "target",
            1,
