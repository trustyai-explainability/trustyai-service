...
TEST_METRIC_VALUE_2 = 0.7  # Second test metric value
...
class TestPrometheusPublisher:
    """Test suite for PrometheusPublisher class."""
    ...
    def test_single_value_gauge_with_request(
        ...
        gauge: Gauge = cast(
            "Gauge",
            publisher.registry._names_to_collectors[metric_name],
        )
    ...
    def test_thread_safety(
        ...
        def create_gauge(thread_id: int) -> None:
            test_id = uuid.uuid4()
    ...
    def test_remove_gauge(
        ...
        # Remove gauge
        publisher.remove_gauge(name=mock_request.metric_name, request_id=test_id)
    ...
    def test_duplicate_gauge_creation(
        ...
        assert publisher.values[test_id1] == TEST_METRIC_VALUE_1
        assert publisher.values[test_id2] == TEST_METRIC_VALUE_2
    ...
    def test_tag_generation_without_request(
        self,
        publisher: PrometheusPublisher,
    ) -> None:
        """Test tag generation without request object."""
        test_id = uuid.uuid4()
        tags = publisher._generate_tags(model_name="test_model", request_id=test_id)
    ...
    def test_invalid_gauge_parameters(
        ...
        with pytest.raises(
            ValidationError,
            match="Either 'value' or 'named_values' must be provided",
        ):
            publisher.gauge(
                GaugeConfig(
                    model_name="test_model",
        ...
        with pytest.raises(
            ValidationError,
            match="Either 'request' or 'metric_name' must be provided",
        ):
            publisher.gauge(
                GaugeConfig(model_name="test_model", request_id=test_id, value=0.5)
            )

    def test_valid_metric_names(self, publisher: PrometheusPublisher) -> None:
    ...
    def test_invalid_metric_names(self, publisher: PrometheusPublisher) -> None:
    ...
    def test_empty_metric_name_validation(self, publisher: PrometheusPublisher) -> None:
    ...
    def test_gauge_creation_with_invalid_metric_name(
        ...
        """Test error handling when creating gauge with invalid metric name."""
    ...
    def test_gauge_config_both_value_and_named_values(
        self,
        publisher: PrometheusPublisher,
        mock_request: MockMetricRequest,
    ) -> None:
        """Test behavior when both value and named_values are provided.

        When both are provided, only one should be used (request-based takes precedence).
        """
        test_id = uuid.uuid4()

        # Both value and named_values provided via request
        # This should work - named_values from request takes precedence
        config = GaugeConfig(
            model_name="test_model",
            request_id=test_id,
            request=mock_request,
            value=0.5,  # This should be ignored when request is provided
            named_values={"metric1": 1.0, "metric2": 2.0},
        )

        # Should not raise - config validation allows this
        publisher.gauge(config)

    def test_gauge_config_error_messages(
        self,
        mock_request: MockMetricRequest,
    ) -> None:
        """Verify validation errors have user-friendly messages."""
        test_id = uuid.uuid4()

        # Test missing value/named_values with request (case 1)
        ...
        assert "Either 'value' or 'named_values'" in error_str

        # Test missing request/metric_name (case 2a)
        with pytest.raises(ValidationError) as exc_info:
            GaugeConfig(model_name="test", request_id=test_id, value=0.5)

        error_str = str(exc_info.value)
        assert "Either 'request' or 'metric_name'" in error_str

        # Test missing value when using metric_name (case 2b)
        with pytest.raises(ValidationError) as exc_info:
            GaugeConfig(
                model_name="test",
                request_id=test_id,
        ...
        error_str = str(exc_info.value)
        assert "Value must be provided when using metric_name" in error_str

    def test_gauge_special_float_values(
        self,
        publisher: PrometheusPublisher,
    ) -> None:
        """Test handling of special float values (NaN, Infinity, negative).

        Prometheus supports these special values, so they should not be rejected.
        """
        test_id = uuid.uuid4()

        # Test negative value
        config_negative = GaugeConfig(
            model_name="test_model",
            request_id=test_id,
            metric_name="test_metric",
            value=-100.5,
        )
        publisher.gauge(config_negative)  # Should not raise

        # Test NaN
        config_nan = GaugeConfig(
            model_name="test_model",
            request_id=test_id,
            metric_name="test_metric",
            value=math.nan,
        )
        publisher.gauge(config_nan)  # Should not raise

        # Test positive infinity
        config_inf = GaugeConfig(
            model_name="test_model",
            request_id=test_id,
            metric_name="test_metric",
            value=math.inf,
        )
        publisher.gauge(config_inf)  # Should not raise

        # Test negative infinity
        config_neg_inf = GaugeConfig(
            model_name="test_model",
            request_id=test_id,
            metric_name="test_metric",
            value=-math.inf,
        )
        publisher.gauge(config_neg_inf)  # Should not raise
