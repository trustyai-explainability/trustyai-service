...
SECONDS_30 = 30
SECONDS_60 = 60
SECONDS_300 = 300  # 5 minutes
SECONDS_1800 = 1800  # 30 minutes
SECONDS_3600 = 3600  # 1 hour
SECONDS_5400 = 5400  # 1.5 hours
SECONDS_7200 = 7200  # 2 hours
SECONDS_86400 = 86400  # 1 day
SECONDS_95445 = 95445  # 1 day + 2 hours + 30 minutes + 45 seconds
SECONDS_172800 = 172800  # 2 days

# Default configuration values
DEFAULT_BATCH_SIZE = 200
DEFAULT_SCHEDULE_SECONDS = 30
...
class MockMetricRequest(BaseMetricRequest):
    ...
    def __init__(
        ...
        """Initialize mock metric request with model ID and metric name."""
        super().__init__(
            model_id=model_id,
    ...
    def retrieve_tags(self) -> dict[str, str]:
        """Return custom tags for the metric request."""
        return {"custom_tag": "custom_value"}


class TestPrometheusScheduler:
    """Test suite for PrometheusScheduler class."""
    ...
    def mock_publisher(self) -> Mock:
        """Create mock PrometheusPublisher."""
    ...
    def test_service_config_parsing(self) -> None:
        ...
        with patch.dict(
            ...
            assert config["metrics_schedule"] == SECONDS_60
            assert config["batch_size"] == DEFAULT_BATCH_SIZE

        with patch.dict("os.environ", {"SERVICE_METRICS_SCHEDULE": "5m"}):
            config = PrometheusScheduler.get_service_config()
            assert config["metrics_schedule"] == SECONDS_300

        with patch.dict("os.environ", {"SERVICE_METRICS_SCHEDULE": "1h"}):
            config = PrometheusScheduler.get_service_config()
            assert config["metrics_schedule"] == SECONDS_3600

    def test_parse_interval_iso8601_seconds(self) -> None:
        """Test parsing ISO-8601 duration with seconds."""
        assert PrometheusScheduler._parse_schedule_interval("PT30S") == SECONDS_30
        assert PrometheusScheduler._parse_schedule_interval("PT1S") == 1
        assert PrometheusScheduler._parse_schedule_interval("PT60S") == SECONDS_60

    def test_parse_interval_iso8601_minutes(self) -> None:
        """Test parsing ISO-8601 duration with minutes."""
        assert PrometheusScheduler._parse_schedule_interval("PT5M") == SECONDS_300
        assert PrometheusScheduler._parse_schedule_interval("PT1M") == SECONDS_60
        assert PrometheusScheduler._parse_schedule_interval("PT30M") == SECONDS_1800

    def test_parse_interval_iso8601_hours(self) -> None:
        """Test parsing ISO-8601 duration with hours."""
        assert PrometheusScheduler._parse_schedule_interval("PT2H") == SECONDS_7200
        assert PrometheusScheduler._parse_schedule_interval("PT1H") == SECONDS_3600
        assert PrometheusScheduler._parse_schedule_interval("PT24H") == SECONDS_86400

    def test_parse_interval_iso8601_days(self) -> None:
        """Test parsing ISO-8601 duration with days."""
        assert PrometheusScheduler._parse_schedule_interval("P1D") == SECONDS_86400
        assert PrometheusScheduler._parse_schedule_interval("P2D") == SECONDS_172800
    ...
    def test_parse_interval_iso8601_combined(self) -> None:
        ...
        assert (
            PrometheusScheduler._parse_schedule_interval("P1DT2H30M45S")
            == SECONDS_95445
        )
        # 1 hour and 30 minutes = 5400 seconds
        assert PrometheusScheduler._parse_schedule_interval("PT1H30M") == SECONDS_5400

    def test_parse_interval_iso8601_fractional(self) -> None:
        """Test parsing ISO-8601 duration with fractional values."""
        assert PrometheusScheduler._parse_schedule_interval("PT0.5H") == SECONDS_1800
        assert PrometheusScheduler._parse_schedule_interval("PT0.25M") == SECONDS_15
    ...
    def test_parse_interval_simple_format_seconds(self) -> None:
        ...
        assert PrometheusScheduler._parse_schedule_interval("30s") == SECONDS_30
        assert PrometheusScheduler._parse_schedule_interval("1s") == 1
        assert PrometheusScheduler._parse_schedule_interval("60s") == SECONDS_60

    def test_parse_interval_simple_format_minutes(self) -> None:
        """Test parsing simple format with minutes (backward compatibility)."""
        assert PrometheusScheduler._parse_schedule_interval("5m") == SECONDS_300
        assert PrometheusScheduler._parse_schedule_interval("1m") == SECONDS_60
        assert PrometheusScheduler._parse_schedule_interval("30m") == SECONDS_1800

    def test_parse_interval_simple_format_hours(self) -> None:
        """Test parsing simple format with hours (backward compatibility)."""
        assert PrometheusScheduler._parse_schedule_interval("2h") == SECONDS_7200
        assert PrometheusScheduler._parse_schedule_interval("1h") == SECONDS_3600
        assert PrometheusScheduler._parse_schedule_interval("24h") == SECONDS_86400

    def test_parse_interval_simple_format_days(self) -> None:
        """Test parsing simple format with days (backward compatibility)."""
        assert PrometheusScheduler._parse_schedule_interval("1d") == SECONDS_86400
        assert PrometheusScheduler._parse_schedule_interval("2d") == SECONDS_172800
    ...
    def test_service_config_iso8601_format(self) -> None:
        ...
        with patch.dict(
            "os.environ",
            {"SERVICE_METRICS_SCHEDULE": "PT5M", "SERVICE_BATCH_SIZE": "200"},
        ):
            config = PrometheusScheduler.get_service_config()
            assert config["metrics_schedule"] == SECONDS_300
            assert config["batch_size"] == DEFAULT_BATCH_SIZE
        ...
        with patch.dict("os.environ", {"SERVICE_METRICS_SCHEDULE": "P1D"}):
            config = PrometheusScheduler.get_service_config()
            assert config["metrics_schedule"] == SECONDS_86400

        with patch.dict("os.environ", {"SERVICE_METRICS_SCHEDULE": "PT1H30M"}):
            config = PrometheusScheduler.get_service_config()
            assert config["metrics_schedule"] == SECONDS_5400
    ...
    def test_service_config_invalid_schedule_falls_back(self) -> None:
        ...
        with patch.dict("os.environ", {"SERVICE_METRICS_SCHEDULE": "invalid_format"}):
            config = PrometheusScheduler.get_service_config()
            assert config["metrics_schedule"] == DEFAULT_SCHEDULE_SECONDS

    @pytest.mark.asyncio
    async def test_register_request(
        self,
        scheduler: PrometheusScheduler,
        mock_request: MockMetricRequest,
    ) -> None:
    ...
    async def test_register_multiple_requests(
        ...
        assert len(scheduler.requests["test_metric"]) == EXPECTED_REQUEST_COUNT
    ...
    async def test_delete_request(
        ...
        await scheduler.delete(metric_name="test_metric", request_id=test_id)
        assert test_id not in scheduler.requests["test_metric"]

        # Verify publisher.remove_gauge was called
        scheduler.publisher.remove_gauge.assert_called_once_with("test_metric", test_id)  # type: ignore[attr-defined]
    ...
    async def test_calculate_with_no_requests(
        ...
        # MODEL_COUNT_TOTAL (1 publish) + MODEL_OBSERVATIONS_TOTAL per model
        # (2 publishes)
        assert scheduler.publisher.gauge.call_count == EXPECTED_CALL_COUNT
    ...
    async def test_thread_safety(self, scheduler: PrometheusScheduler) -> None:
        ...
        async def register_request(thread_id: int) -> None:
    ...
    async def test_calculate_with_multiple_requests_same_metric(
        ...
        scheduler_with_publisher.metrics_directory.register(
            name="spd",
            calculator=mock_spd_calculator,  # type: ignore[arg-type]
        )
