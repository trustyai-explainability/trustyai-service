...
class TestSerializeRows:
    ...
    def test_serialize_simple_rows(self) -> None:
    ...
    def test_serialize_exceeds_max_size(self) -> None:
    ...
    def test_dynamic_void_size(self) -> None:
        ...
        row_sizes = [
            len(gzip.compress(json.dumps(row, default=json_encoder).encode("utf-8")))
            for row in rows
        ]
    ...
    def test_serialized_data_is_gzip(self) -> None:
...
class TestDeserializeRows:
    ...
    def test_deserialize_json_gzip_simple(self) -> None:
    ...
    def test_deserialize_json_gzip_mixed_types(self) -> None:
    ...
    def test_deserialize_with_padding(self) -> None:
    ...
    def test_invalid_gzip_json_data(self) -> None:
        ...
        with pytest.raises(
            ValueError, match="Failed to deserialize row as gzip-compressed JSON"
        ):
            deserialize_rows(serialized)

    def test_roundtrip_preserves_data_types(self) -> None:
...
class TestDecompressionSafety:
    ...
    def test_deserialize_corrupted_gzip_row(self) -> None:
    ...
    def test_deserialize_non_gzip_row_rejected(self) -> None:
        ...
        with pytest.raises(
            ValueError, match=r"Unsupported serialization format|Expected gzip"
        ):
            deserialize_rows(void_array)

    def test_large_but_valid_rows(self) -> None:
        ...
        # Use a larger limit for this test since the serialized data is ~1892 bytes
        serialized = serialize_rows(large_row, max_void_type_length=2048)
        deserialized = deserialize_rows(serialized)

        assert len(deserialized) == 1
        assert list(deserialized[0]) == large_row[0]

    def test_mixed_row_sizes(self) -> None:
    ...
    def test_compression_effectiveness(self) -> None:
        ...
        serialized = serialize_rows(
            repetitive_row, max_void_type_length=MAX_VOID_TYPE_LENGTH
        )
