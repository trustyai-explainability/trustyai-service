...
class MetricValueCarrier:
    ...
    def get_value(self) -> float:
        if self.single:
            ...
            raise UnsupportedOperationException("Metric value is not singular and therefore must be accessed via .get_value()")
    
    def get_named_values(self) -> Dict[str, float]:
        if not self.single:
            return self.named_values
        else:
            raise UnsupportedOperationException("Metric value is singular and therefore must be accessed via .get_named_values()")
