...
from trustyai_service.core.metrics.drift.mmd import (
    DEFAULT_ALPHA,
    DEFAULT_BANDWIDTH,
    DEFAULT_KERNEL,
...
class MMDMetricRequest(BaseMetricRequest):
    ...
    def _set_default_metric_name(self) -> "MMDMetricRequest":
...
async def compute_mmd(
    ...
    if missing:
        raise HTTPException(
            ...
            detail=f"Features not found in data: {', '.join(missing)}",
        )
...
@router.get(routes.DRIFT_MMD.definition)
                ...
                requests_list.append(
                    {
                        ...
                        "metricName": request_metric_name,
...
@router.post(routes.DRIFT_FOURIER_MMD.compute, deprecated=True)
...
@router.get(routes.DRIFT_FOURIER_MMD.definition, deprecated=True)
...
@router.post(routes.DRIFT_FOURIER_MMD.request, deprecated=True)
async def schedule_fouriermmd(request: MMDMetricRequest) -> dict[str, str]:
...
@router.delete(routes.DRIFT_FOURIER_MMD.request, deprecated=True)
...
async def calculate_mmd_metric(
    ...
    return MetricValueCarrier(result["statistic"])
