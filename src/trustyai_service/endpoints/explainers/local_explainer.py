...
# LIME configuration bounds
MAX_NUM_SAMPLES = 100000
MAX_N_TRAINING_ROWS = 1000000
MAX_KERNEL_WIDTH = 10.0
MAX_NUM_FEATURES = 1000
MAX_TIMEOUT_SECONDS = 3600
METADATA_CHUNK_SIZE = 1000
MAX_METADATA_ROWS = 1000000
...
class LimeExplainerConfig(BaseModel):
    ...
    seed: int | None = Field(
        default=None,
        description=(
            "Random seed for reproducible explanations and confidence intervals."
            " If omitted, results are nondeterministic."
        ),
    )
    ...
    def validate_num_samples(cls, v: int) -> int:
        """Ensure num_samples is positive and bounded."""
        if v < 1:
            msg = "num_samples must be >= 1"
            raise ValueError(msg)
        if v > MAX_NUM_SAMPLES:
            msg = f"num_samples must be <= {MAX_NUM_SAMPLES} to prevent resource exhaustion"
            raise ValueError(msg)
        return v

    @field_validator("n_training_rows")
    @classmethod
    def validate_n_training_rows(cls, v: int) -> int:
        """Ensure n_training_rows is positive and bounded."""
        if v < 1:
            msg = "n_training_rows must be >= 1"
            raise ValueError(msg)
        if v > MAX_N_TRAINING_ROWS:
            msg = f"n_training_rows must be <= {MAX_N_TRAINING_ROWS} to prevent resource exhaustion"
            raise ValueError(msg)
        return v

    @field_validator("kernel_width")
    @classmethod
    def validate_kernel_width(cls, v: float) -> float:
        """Ensure kernel_width is positive and reasonable."""
        if v <= 0.0:
            msg = "kernel_width must be > 0"
            raise ValueError(msg)
        if v > MAX_KERNEL_WIDTH:
            msg = f"kernel_width must be <= {MAX_KERNEL_WIDTH}"
            raise ValueError(msg)
        return v
    ...
    def validate_num_features(cls, v: int) -> int:
        """Ensure num_features is positive and bounded."""
        if v < 1:
            msg = "num_features must be >= 1"
            raise ValueError(msg)
        if v > MAX_NUM_FEATURES:
            msg = f"num_features must be <= {MAX_NUM_FEATURES}"
            raise ValueError(msg)
        return v
    ...
    def validate_timeout(cls, v: int) -> int:
        """Ensure timeout is positive and deployment-appropriate."""
        if v < 1:
            msg = "timeout must be >= 1 second"
            raise ValueError(msg)
        if v > MAX_TIMEOUT_SECONDS:
            msg = f"timeout must be <= {MAX_TIMEOUT_SECONDS} seconds"
            raise ValueError(msg)
        return v
...
async def get_stored_prediction(
    ...
    # Read metadata in bounded chunks to find prediction_id
    row_idx: int | None = None

    for chunk_offset in range(0, MAX_METADATA_ROWS, METADATA_CHUNK_SIZE):
        metadata_chunk = await storage_interface.read_data(
            metadata_dataset, start_row=chunk_offset, n_rows=METADATA_CHUNK_SIZE
        )

        if metadata_chunk is None or len(metadata_chunk) == 0:
            break

        for i, row in enumerate(metadata_chunk):
...
async def _prepare_training_data(
    ...
    """Fetch organic observations and return (X_raw, y, training_column_names).

    X_raw is not yet cast to float64 — callers must convert in a thread to avoid
    blocking the event loop with large array conversions.
    ...
    x_train_raw = organic_df[training_cols].values
    ...
    return x_train_raw, y_train, training_cols
...
async def local_lime_explanation(
    ...
    x_train_raw, y_train, training_cols = await _prepare_training_data(
    ...
    selected_values = instance[col_idx]

    # Validate selected feature values are numeric before converting to float64
    try:
        instance_f64 = selected_values.astype(np.float64)
    except (ValueError, TypeError) as exc:
        msg = f"Invalid feature data: cannot convert to numeric. {exc}"
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST,
            detail=msg,
        ) from exc
    ...
    mode = (
        "regression" if request.config.model.target == "regressor" else "classification"
    )
    ...
    def _compute() -> tuple[
        ...
        _surrogate = build_surrogate(x_train, y_train, mode=mode)
