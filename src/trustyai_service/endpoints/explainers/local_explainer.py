...
_MAX_SHAP_N_SAMPLES = 50_000
_MAX_SHAP_N_TRAINING_ROWS = 1_000_000
_MAX_SHAP_TIMEOUT_SECONDS = 3600
...
class SHAPExplainerConfig(BaseModel):
    ...
    def validate_n_samples(cls, v: int) -> int:
        """Ensure n_samples is positive and bounded to prevent resource exhaustion."""
        if v < 1:
            msg = "n_samples must be >= 1"
            raise ValueError(msg)
        if v > _MAX_SHAP_N_SAMPLES:
            msg = f"n_samples must be <= {_MAX_SHAP_N_SAMPLES} to prevent resource exhaustion"
            raise ValueError(msg)
        return v
    ...
    def validate_n_training_rows(cls, v: int) -> int:
        ...
        if v < 1:
            ...
            raise ValueError(msg)
        if v > _MAX_SHAP_N_TRAINING_ROWS:
            msg = f"n_training_rows must be <= {_MAX_SHAP_N_TRAINING_ROWS} to prevent resource exhaustion"
            raise ValueError(msg)
        return v
    ...
    def validate_timeout(cls, v: int) -> int:
        ...
        if v < 1:
            ...
            raise ValueError(msg)
        if v > _MAX_SHAP_TIMEOUT_SECONDS:
            msg = f"timeout must be <= {_MAX_SHAP_TIMEOUT_SECONDS} seconds"
            raise ValueError(msg)
        return v
...
_REG_MAP: dict[RegularizerType, str | float] = {
    ...
    # 0.0 disables L1 feature selection without setting max_iter=0 (which empties the active set).
    RegularizerType.NONE: 0.0,
}
...
async def local_shap_explanation(
    ...
    # Use explicit task mode from request rather than inferring from dtype.
    shap_mode = (
        "regression" if request.config.model.target == "regressor" else "classification"
    )

    # Validate LOGIT link compatibility: regressor outputs must be in [0, 1].
    # Classifier outputs use predict_proba (handled in shap.py), so no issue there.
    if config.link == LinkType.LOGIT and shap_mode == "regression":
        out_min, out_max = float(y_train.min()), float(y_train.max())
        if out_min < 0.0 or out_max > 1.0:
            raise HTTPException(
                status_code=HTTPStatus.BAD_REQUEST,
                detail=(
                    f"link=LOGIT requires model outputs in [0, 1], "
                    f"but stored outputs range [{out_min:.4g}, {out_max:.4g}]. "
                    "Use link=IDENTITY for regression targets outside this range."
                ),
            )

    def _compute() -> tuple[np.ndarray, np.ndarray | None, np.ndarray | None]:
        _surrogate = build_surrogate(x_train, y_train, mode=shap_mode)
