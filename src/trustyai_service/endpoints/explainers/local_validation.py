...
def sanitize_validation_errors(
    ...
    """Remove raw inputs and exception context from every sensitive error.

    Parent-object inputs, such as those attached to a missing-field error, can
    embed a credential-bearing ``base_url`` even when the error location does
    not mention that field.
    """
    return [
        {key: value for key, value in error.items() if key not in _SENSITIVE_ERROR_KEYS}
        for error in errors
    ]
