...
def compute_lime_explanation(
    ...
    # Select the label used for all per-label values consistently.
    # Regression stores results at sentinel label 1; classification uses the first
    # available label (the predicted class for the instance).
    label = (
        explanation.available_labels()[0] if explanation.mode == "classification" else 1
    )
    ...
    r2_score = _scalar(explanation.score)
    local_pred = _scalar(explanation.local_pred)
    intercept = _scalar(explanation.intercept)
...
def compute_lime_confidence_intervals(
    ...
    for _ in range(n_bootstrap):
        try:
            ...
            iteration_values = dict(explanation.as_list())
            for feature_name in feature_names:
                feature_values.setdefault(feature_name, []).append(
                    float(iteration_values.get(feature_name, 0.0))
                )
