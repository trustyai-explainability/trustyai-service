    ...
    *,
    ...
    Args:
        ...
        mode: Explicit task mode (``"regression"`` or ``"classification"``).
            Always pass this explicitly — dtype inference is unreliable for
            integer-valued regression or float-encoded classification targets.
            If ``None``, falls back to dtype inference (deprecated path).
    ...
    """
    if mode is None:
        # Fallback: infer from dtype. Unreliable for integer regression targets
        # or float-encoded classification labels — always pass mode explicitly.
        mode = (
            "regression"
...
def _get_predict_fn(surrogate: BaseEstimator, link: str) -> Any:  # noqa: ANN401
    """Return the correct prediction callable for the given surrogate and link.

    For ``link="logit"``, ``KernelExplainer`` expects outputs in ``(0, 1)``.
    - Classifier surrogate: use ``predict_proba[:, 1]`` (positive-class probability).
    - Regressor surrogate: use ``predict`` as-is; caller must ensure [0, 1] outputs.

    For ``link="identity"``, ``predict`` is always correct.
    """
    if link == "logit" and isinstance(surrogate, RandomForestClassifier):
        return lambda x: surrogate.predict_proba(x)[:, 1]
...
def compute_shap_values(
    ...
    explainer = shap.KernelExplainer(
        _get_predict_fn(surrogate, link), bg_summary, link=link
    )
