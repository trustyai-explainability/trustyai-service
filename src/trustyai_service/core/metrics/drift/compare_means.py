...
Detects drift by comparing means between reference and current
distributions using independent two-sample t-tests (Student's t-test or
Welch's t-test).
"""
...
DEFAULT_EQUAL_VAR = (
    False  # Use Welch's t-test by default (does not assume equal variances)
)
...
class CompareMeans:
    ...
    Detects drift by comparing means between reference and current
    distributions using independent two-sample t-tests. By default, uses
    Welch's t-test (equal_var=False), which does not assume equal
    ...
    def ttest_ind(
        ...
        *,
        ...
        :param equal_var: If True, perform a standard Student's t-test that
            assumes equal population variances. If False (default), perform
            Welch's t-test, which does not assume equal variances.
        :param nan_policy: How to handle NaN values. Options: 'propagate',
            'raise', 'omit' (default).
